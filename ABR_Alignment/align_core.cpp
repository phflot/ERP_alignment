		...
		while (iteration_counter++ < iterations) {
            ...
            for (int j = 1; j < ny - 1; j++) {
				...
				for (int k = 0; k < n_channels; k++) {
					...
					num_v -= weight * psi[nd_idx] * (fy[nd_idx] * fyy[nd_idx] - f_ref_y[ref_idx] * fyy[nd_idx]);

					denom_v += weight * psi[nd_idx] * fyy[nd_idx] * fyy[nd_idx];
				}
...
void nonlinearity(double *psi,
	...
	for (int k = 0; k < n_channels; k++) {
			for (int j = 0; j < ny; j++) {
				...
				tmp = fy[nd_idx] + fyy[nd_idx] * dv[nd_idx] - f_ref[ref_idx];
				tmp = tmp < 0 ? 0 : tmp * tmp;
